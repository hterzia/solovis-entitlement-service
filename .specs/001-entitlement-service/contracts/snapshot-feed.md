...
**What is deliberately *not* replicated is everything only an explanation needs.** Overrides travel as `(account, capability, kind, value)`. Their `reason`, `createdBy`, `createdAt` and id stay in the management layer, because only the operator UI renders the §6.1 trace and reason text is commercially sensitive — `future-spec.md` §11 warns that internal reasons like *"suspended pending investigation"* must not reach surfaces built for other audiences. A consumer that needs an explanation calls `GET /v1/accounts/{id}/capabilities/{key}`, which holds the complete record. See `plan.md`, "Recorded interpretations", for how this reads against criterion 21.
