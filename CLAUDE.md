...
Built and tested: `entitlement-core` (model, order, resolver + explain, snapshot, mutator, conformance), `entitlement-service/store` (dual-pool SQLite config, `V1__baseline.sql`, one repository per table), the service's `api/`, `admin/`, `snapshot/`, `audit/`, `error/`, `seed/` packages, and `entitlement-client` (replica, conformance gate, snapshot poller, disk cache, decision/explain/read-through paths, metrics seam). `entitlement-service` carries a test-scoped dependency on `entitlement-client` for `ClientAgainstRealFeedTest`, which proves the SDK against the real running service rather than a hand-written stub. Not yet built: the operator SPA (`.specs/001-entitlement-service/frontend-plan.md`, in progress in the `worktree-entitlement-ui-frontend` worktree) and `entitlement-loadtest`.
