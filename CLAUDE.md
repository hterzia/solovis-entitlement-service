...
- **`DemoDataSeeder` is an `InitializingBean`**, conditional on `entitlement.seed.enabled`, and writes through the real admin services so it can never declare data the validation rules reject. It applies `resources/seed/demo-seed.json` — 16 capabilities, 5 plans, 61 accounts, 28 events — in **one chronological pass**, with the `Clock` bean wound to each authored day.
  - **Not an `ApplicationRunner`, for two reasons.** Runners fire after the web connector is already accepting traffic, so one would leave a window where the console is reachable and half populated. And `@Scheduled` tasks start with the context lifecycle, *also* after every `InitializingBean` — which is what keeps `WindowBoundaryRoller` (`initialDelay = 0`, reads `LocalDate.now(clock)`) from ever seeing the wound clock. As a runner it would fire mid-seed, record `window.rolledThrough` in the fictional past, then publish a flood of boundary transitions for moments nobody observed. It needs no `@DependsOn`: `SnapshotPublisher` takes its version from the `snapshot_version` autoincrement, so a migrated schema is the only precondition.
  - **The wound clock is what makes all four override standings seedable.** c7 refuses a window that has already ended, so a seeder standing in the present cannot manufacture an `ENDED` override at all; wound to the authored day it is an ordinary validated write. `SeedClock` decorates `ClockConfig.base(ZoneId)` rather than building its own — `NoDirectClockAccessTest` exempts only `ClockConfig.java`, and a clock over UTC instead of the service zone would compile while silently shifting every window boundary. It is released in a `finally` before the connector opens.
  - **One chronological pass, not four section-major ones.** `seq` order and `occurred_at` order must agree (see the `audit_event` note above); iterating capabilities-then-plans-then-accounts-then-events put an early `occurred_at` on a high `seq`. Steps are merged and sorted by day, tie-broken so a capability precedes the plan entitling it. The last day maps to *now*, not to 9am, or the current `snapshot_version.publishedAt` reads hours stale to every polling replica.
  - **Seeded-ness is recorded, not inferred** — `seed.started` / `seed.completed` in `service_state`. The old "are there any plans?" check let a crash partway through leave a permanently half-populated demo that every later boot skipped in silence; a started-but-never-completed marker now fails startup, and the recovery is to delete the database file, because a backdated history has to be written in time order into an empty one.
...
Built and tested as of 2026-08-10, all green: **645 backend** (core 124, client 168, service 353), **209 frontend unit**, **33 end-to-end**. Feature 002 (time-bounded overrides and point-in-time answers) is merged; its plan is `.specs/002-time-bound-override/plan.md`. Count backend tests from a clean run — `target/surefire-reports/` keeps stale files from single-class runs and will over-report otherwise.
