...
public class OverrideAdminController {
    ...
    /** What the decision becomes if this override were removed — read-only (ui-screens.md, screen 3). */
    @GetMapping("/{id}/removal-preview")
    public DecisionResponseDto removalPreview(@PathVariable String external, @PathVariable String id) {
        return service.previewRemoval(external, id);
    }
