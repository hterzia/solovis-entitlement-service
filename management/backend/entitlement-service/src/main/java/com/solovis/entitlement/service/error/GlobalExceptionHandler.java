...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public GlobalExceptionHandler(DecisionReadDao decisionReadDao) {
    ...
    private Optional<String> snapshotVersionHeader(String requestUri) {
        ...
        try {
            return Optional.of(String.valueOf(decisionReadDao.latestVersion()));
        } catch (RuntimeException databaseUnreachable) {
            // An empty snapshot_version table answers 0 rather than throwing, so this is only
            // reached if the read pool itself is unavailable — in which case an error response
            // missing one header is a far better outcome than the error handler itself throwing.
            return Optional.empty();
        }
    }
